...
#include "html_content_hash_utils.h"
...
bool MainWindow::show_inital()
{
  ...
  if (!gui_utils::calculate_html_folder_hash(m_html_path, runtime_hash) || runtime_hash != std::string(HTML_CONTENT_HASH))
...
QString MainWindow::is_html_verified(const QString& param)
{
  TRY_ENTRY();
  std::string runtime_hash;
  if (!gui_utils::calculate_html_folder_hash(m_html_path, runtime_hash))
  {
